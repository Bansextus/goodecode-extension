...
const LATEST_RELEASE_API_URL = "https://api.github.com/repos/Bansextus/Goodebot-Downloads/releases/latest";
const RELEASES_API_URL = "https://api.github.com/repos/Bansextus/Goodebot-Downloads/releases";
const DIRECT_DOWNLOAD_URL = "https://github.com/Bansextus/Goodebot-Downloads/releases/latest/download/Goodebot-macOS.zip";
const GOODEBOT_DEV_DOWNLOAD_PASSWORD = "56Wrenches.1!";
const GOODEBOT_DEV_ASSET_NAMES = ["Goodebot-dev-macOS.zip", "Goodebot-Dev-macOS.zip", "Goodebot-Dev-dev-macOS.zip"];
...
const GOODEBOT_DEV_LOCAL_ZIPS = [
  path.join(os.homedir(), "GoodebotDevBuilds", "Goodebot", "dev", "Goodebot-dev-macOS.zip"),
  path.join(os.homedir(), "GoodebotDevBuilds", "GoodebotDev", "dev", "Goodebot-Dev-dev-macOS.zip"),
];
const GOODEBOT_ZIP_NAMES = ["Goodebot-macOS.zip", "Goodebot-macOS (1).zip"];
...
function activate(context) {
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand("goodecode.downloadGoodebotDev", async () => {
      await downloadGoodebotDevWithPassword();
      refreshAll();
    }),
...
function renderStudioHtml(webview, extensionUri) {
  ...
  const state = collectStudioState();
  const iconUri = imageDataUri(path.join(__dirname, "media", "goodecode-icon.png"), "image/png");
  const bansextusUri = imageDataUri(path.join(__dirname, "media", "bansextus-343k.jpeg"), "image/jpeg");
...
<html lang="en">
  <head>
    ...
    <style>
      ...
      .credit-mark.vex {
        color: #ffffff;
        background: linear-gradient(135deg, rgba(78, 215, 255, 0.22), rgba(24, 83, 156, 0.34));
      ...
      .credit-logo {
        ...
        border: 1px solid rgba(86, 227, 159, 0.28);
      }
      .tri-chip.green {
        background: rgba(86, 227, 159, 0.13);
        border-color: rgba(86, 227, 159, 0.34);
      }
      .tri-chip.yellow {
        background: rgba(248, 203, 95, 0.12);
        border-color: rgba(248, 203, 95, 0.34);
      }
    ...
    <div class="shell">
      ...
      <section class="card">
        ...
        <div class="credit-row">
          <div class="credit-mark vex" aria-label="VEX">VEX</div>
          ${bansextusUri ? `<img class="credit-logo" src="${escapeAttribute(bansextusUri)}" alt="bansextus(343K):" />` : `<div class="credit-mark">343K</div>`}
          <div class="chip-row">
            <span class="chip">Goodecode blue</span>
            <span class="chip tri-chip yellow">Goodebot yellow</span>
            <span class="chip tri-chip green">Goodebot Dev green</span>
          </div>
        </div>
...
function renderInstallPageCard(state) {
  ...
  const updateLine = state.goodebotInstalled
    ...
    : "";
  const updateButton = state.goodebotInstalled
    ? `<button data-command="updateGoodebot">Update Goodebot</button>`
    : "";
...
function detectGoodebotInstallSource(workspaceRoot) {
  const home = os.homedir();
  const downloadsRoot = path.join(home, "Downloads");
  const candidates = [];

  if (workspaceRoot) {
    candidates.push(
      path.join(workspaceRoot, "builds", "Goodebot", "Goodebot-macOS.zip"),
      path.join(workspaceRoot, "builds", "Goodebot", "Goodebot.app"),
      path.join(workspaceRoot, "Goodebot Installer.app")
    );
  }
  ...
  candidates.push(
    ...
    path.join(GOODEBOT_LOCAL_REPO, "builds", "Goodebot", "Goodebot.app"),
    path.join(GOODEBOT_LOCAL_REPO, "Goodebot Installer.app"),
    path.join(GOODEBOT_DOWNLOAD_REPO, "builds", "Goodebot", "Goodebot-macOS.zip"),
    path.join(GOODEBOT_DOWNLOAD_REPO, "builds", "Goodebot", "Goodebot.app"),
    ...GOODEBOT_ZIP_NAMES.map((name) => path.join(downloadsRoot, name)),
    path.join(downloadsRoot, "Goodebot.app"),
    path.join(downloadsRoot, "Goodebot Installer.app")
  );
...
async function updateGoodebotForMac() {
  ...
  if (!installed) {
    const choice = await vscode.window.showInformationMessage(
      "Goodebot is not installed yet. Install the latest Mac build now?",
      "Install Goodebot",
      "Open Downloads"
    );
    if (choice === "Install Goodebot") {
      await installGoodebotForMacWithOptions({ forceRedownload: true, updateMode: true });
  ...
  const installedBuild = readGoodebotBuildInfo(installed.path);
  let release = null;
  try {
  ...
  if (choice === "Update Goodebot" || choice === "Reinstall Latest") {
    await installGoodebotForMacWithOptions({ forceRedownload: true, updateMode: true, release });
  } else if (choice === "Open Downloads") {
    await vscode.env.openExternal(vscode.Uri.parse(release.htmlUrl || DOWNLOAD_URL));
  }
}

async function downloadGoodebotDevWithPassword(providedPassword) {
  if (process.platform !== "darwin") {
    await vscode.window.showWarningMessage("Goodebot Dev downloads are currently for macOS only.");
    return;
  }

  const password = typeof providedPassword === "string"
    ? providedPassword
    : await vscode.window.showInputBox({
        title: "Download Goodebot Dev",
        prompt: "Enter the Goodebot Dev download password.",
        password: true,
        ignoreFocusOut: true,
        validateInput: (value) => value ? null : "Enter the password.",
      });
  ...
  if (password !== GOODEBOT_DEV_DOWNLOAD_PASSWORD) {
    await vscode.window.showErrorMessage("Incorrect Goodebot Dev password.");
    return;
  }
  ...
  await vscode.window.withProgress(
    {
      ...
      title: "Downloading Goodebot Dev",
      cancellable: false,
    },
    async (progress) => {
      progress.report({ message: "Finding the latest Goodebot Dev build..." });
      const devRelease = await fetchLatestGoodebotDevRelease();
      ...
      if (!devRelease?.assetUrl && localDevZip) {
        ...
        const choice = await vscode.window.showInformationMessage(
          `Goodebot Dev copied from the local dev folder to ${targetPath}`,
          "Reveal Zip"
        );
        if (choice === "Reveal Zip") {
          await revealPathInOS(targetPath);
        }
        return;
      }

      if (!devRelease?.assetUrl) {
        const choice = await vscode.window.showWarningMessage(
          "No Goodebot Dev zip asset was found in GitHub releases or the local dev folder.",
          "Open Releases"
        );
      ...
      progress.report({ message: `Downloading ${devRelease.assetName}...` });
      await downloadFile(devRelease.assetUrl, targetPath);

      const choice = await vscode.window.showInformationMessage(
        `Goodebot Dev downloaded from ${devRelease.tagName || "release"} to ${targetPath}`,
        "Reveal Zip",
        "Open Releases"
      );
      if (choice === "Reveal Zip") {
        ...
        await vscode.env.openExternal(vscode.Uri.parse(devRelease.htmlUrl || DOWNLOAD_URL));
      }
    }
  ).catch(async (error) => {
    await vscode.window.showErrorMessage(`Goodebot Dev download failed: ${error.message}`);
  });
}
...
async function installGoodebotForMacWithOptions(options) {
  ...
  await vscode.window.withProgress(
    ...
    async (progress) => {
      ...
      fs.mkdirSync(path.dirname(GOODEBOT_DESTINATION_APP), { recursive: true });
      ...
      if (options.forceRedownload || !resolvedSource) {
        ...
        resolvedSource = {
          ...
          label: path.basename(downloadZipPath),
          kind: "zip",
        };
      }
      ...
      const backupPath = await backupInstalledGoodebotApp(installed, state);
      ...
      progress.report({ message: "Copying Goodebot into Applications..." });
      fs.rmSync(GOODEBOT_DESTINATION_APP, { recursive: true, force: true });
      await execFileAsync("ditto", [appSourcePath, GOODEBOT_DESTINATION_APP]);

      progress.report({ message: "Removing macOS quarantine flags..." });
      await execFileAsync("xattr", ["-dr", "com.apple.quarantine", GOODEBOT_DESTINATION_APP]).catch(() => {});

      progress.report({ message: "Launching Goodebot..." });
      await openTarget(GOODEBOT_DESTINATION_APP);
      const legacyNote = legacyPath ? ` Legacy file: ${legacyPath}` : "";
      await vscode.window.showInformationMessage(`Goodebot installed at ${GOODEBOT_DESTINATION_APP}.${legacyNote}`);
    }
  ).catch(async (error) => {
...
async function fetchLatestGoodebotDevRelease() {
...
function goodebotDownloadFailureMessage(error) {
  ...
  if (message.includes("status 404") || message.includes("status 403")) {
    return "Goodebot could not download a public Mac zip from GitHub. Put Goodebot-macOS.zip or Goodebot.app anywhere on this Mac, then click Install again.";
  }
...
function writeGoodebotPersonalizationSnapshot(state, targetBuild = {}) {
  const snapshot = {
...
function writeGoodebotLegacyMigrationFile(state, targetBuild = {}, details = {}) {
  ...
  const migration = {
    ...
    oldGoodebot: {
      appPath: state?.goodebotInstalled?.path || "",
      build: state?.goodebotInstalledBuild || {},
    },
    targetGoodebot: {
      build: targetBuild,
      installPath: GOODEBOT_DESTINATION_APP,
